...
**Statut** : Phase 8 — pipeline d'import construit et testé ; **import réel exécuté** pour les 13 entreprises françaises approuvées (statut `draft`, non publiées) ; **vérification SIRENE des 87 entreprises restantes réalisée** (aucune n'a de SIREN exploitable dans la source, aucune importée) ; migration `0021_industry_mapping_and_content_source.sql` **appliquée sur Supabase** ; **secteur (`company_industries`) et description éditoriale (`company_translations`) écrits pour les 5 fiches pilotes** (FIGEAC AERO, Safran, Airbus Atlantic, STMicroelectronics Rousset SAS, MAF AGROBOTIC), toutes **restées en statut `draft`, non publiées** ; en attente d'autorisation explicite de publication (aucune des 13 n'est publique, aucune offre/besoin/opportunité touché).
...
8. Audit des données de sourcing + pipeline d'import (`data/raw/`) — **audit et pipeline faits** (Phase 8, voir docs/DATA_INVENTORY.md, docs/DATA_MAPPING.md, docs/DATA_SOURCES.md, docs/IMPORT_PIPELINE.md) : garde-fou de licence, staging, dédoublonnage, dry run testés réellement. Dry run exécuté sur les 13 entreprises françaises approuvées ; **import réel non exécuté**, en attente d'autorisation séparée. Base Québec non importée (docs/QUEBEC_SOURCING_STRATEGY.md, documentation seulement). Explicitement reporté à une phase ultérieure sur demande du propriétaire du projet : le badge « entreprise vérifiée » avancé (`company_verifications`).
...
_Prochaine révision prévue : après votre décision sur l'import réel du lot pilote._
