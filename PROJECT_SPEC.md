...
**Statut** : Phase 10C — corrections de sécurité issues d'un audit indépendant Cowork, en 4 lots (2026-09-20/21) : détection de doublon avant création d'entreprise + limitation anti-rafale des demandes de mise en relation (migration `0024`) ; déplacement des coordonnées professionnelles (`professional_email`/`phone`) vers une table dédiée `company_contacts`, jamais publique, corrigeant leur affichage jusque-là réel sur la fiche publique (migration `0025`) ; publication d'entreprise sécurisée via une nouvelle table `company_publication_requests` et deux RPC SECURITY DEFINER (`request_company_publication()`/`review_company_publication_request()`), corrigeant une faille qui permettait à un owner/admin de rendre sa propre entreprise `active` par un simple `UPDATE` (migration `0026`, **appliquée sur Supabase le 2026-09-21, tests d'intégration verts 24/24**). Les migrations `0024` et `0025` sont également appliquées en production — confirmé le 2026-09-23 par vérification en lecture seule des objets réels (existence des fonctions/table, absence de valeur legacy), mais **sans réexécution récente de leurs suites d'intégration dédiées** contre l'état actuel de la base (dette documentée, voir CHANGELOG.md). Détail complet des 4 lots dans CHANGELOG.md — cette entrée a été rédigée le 2026-09-23, après les commits, car la documentation n'avait pas suivi au moment de la Phase 10C (rupture ponctuelle avec la pratique habituelle du projet). Phase 9 (mise en relation commerciale, `partnership_requests`) et Phase 8 (import réel des 13 entreprises françaises pilotes, 3 publiées) restent inchangées sur le fond, sous réserve des redéfinitions de fonctions listées ci-dessus — voir §12 et le point 8 de §16.
...
2. ✅ **fait (2026-09-24)** — **Plans de retour arrière testés en préprod** pour les migrations `0024` (dédoublonnage + anti-rafale), `0025` (protection des coordonnées professionnelles) et `0026` complet (voir `docs/rollback/README.md`, `docs/rollback/0024-dedup-et-anti-rafale.sql`, `0025-protection-coordonnees.sql`, `0026-complet.sql`). Testés de bout en bout contre `fcp-preprod` : retours arrière 0026→0025→0024 exécutés réellement, schéma exposé identique à l'état post-0023 (zéro écart), 3 contacts fictifs recopiés `company_contacts` → `companies` intacts, migrations réappliquées 0024→0025→0026, schéma revenu identique à l'état de départ, suite d'intégration complète 224/224. Dette de la Phase 10C résolue.
...
**partnership_requests** _(✅ construite en Phase 9 — mécanisme dédié, distinct de `opportunity_responses` (§4.5) et de `company_offers`/`company_needs` (§4.3) ; voir docs/PARTNERSHIP_REQUESTS.md et docs/SECURITY.md)_

- id, requester_company_id, target_company_id (toujours entreprise↔entreprise, jamais utilisateur↔entreprise ; jamais vers soi-même), created_by, status (`pending`/`pending_unclaimed`/`accepted`/`declined`/`withdrawn`/`expired`), subject, message (tous deux obligatoires, jamais générés depuis le matching), source_type (`DIRECTORY`/`MATCH`/`OPPORTUNITY`/`OTHER`), source_match_id (FK `matches`, nullable), source_opportunity_id (FK `opportunities`, nullable), created_at, updated_at, responded_at, responded_by, withdrawn_at.
- Une seule demande active (`pending`/`pending_unclaimed`) à la fois par paire d'entreprises ; nouvelle demande possible après refus/retrait/expiration.
- Provenance MATCH/OPPORTUNITY : une **référence** vers l'artefact déjà calculé, jamais une copie ni un recalcul du score ; intégrité vérifiée côté serveur (le match ou l'opportunité doit réellement concerner les deux entreprises de la demande).
- Toute écriture passe par 4 fonctions SECURITY DEFINER (`create_partnership_request()`, `accept_partnership_request()`, `decline_partnership_request()`, `withdraw_partnership_request()`) — aucune politique RLS d'insertion/mise à jour, même principe que `company_claims` (Phase 7).
- Entreprise cible non revendiquée : statut `pending_unclaimed`, aucune notification automatique ; devient `pending` et notifiée dès que l'entreprise est revendiquée, via un déclencheur sur `companies.claimed_at` qui laisse `submit_company_claim()`/`review_company_claim()` (Phase 7) strictement inchangées.
...
Tables encore à ajouter, avec leur propre phase : `company_verifications` (11), `favorites` (report indéterminé, plus rattachée à une phase numérotée depuis que la mise en relation a été traitée séparément en Phase 9), `subscriptions` (12), `user_consents`/`data_subject_requests` (au plus tard avant le lancement commercial). `import_batches`/`staging_companies`/`import_row_issues`/`import_duplicate_candidates` construites en Phase 8 (migration 0020). `partnership_requests` construite en Phase 9 (migration 0023).
...
| 2026-09-20 | `partnership_requests` créée comme mécanisme dédié, jamais une réutilisation de `opportunity_responses`/`company_offers`/`company_needs` | Une demande de mise en relation représente une intention humaine explicite entre deux entreprises, distincte d'une réponse à une opportunité publiée ou d'une déclaration d'offre/besoin (§1 de la demande Phase 9) |
| 2026-09-20 | Écriture de `partnership_requests` réservée à 4 fonctions SECURITY DEFINER, aucune politique RLS d'insertion/mise à jour | Même principe que `company_claims` (Phase 7) : rend structurellement impossible de fabriquer une demande en changeant `requester_company_id` côté client |
| 2026-09-20 | Provenance MATCH/OPPORTUNITY vérifiée côté serveur (le match ou l'opportunité doit réellement concerner les deux entreprises), jamais supposée depuis les paramètres reçus du client | Sans ce contrôle, un client pourrait faire référencer à sa demande un match ou une opportunité appartenant à d'autres entreprises |
| 2026-09-20 | Entreprise cible non revendiquée gérée par un NOUVEAU déclencheur sur `companies.claimed_at`, sans modifier `submit_company_claim()`/`review_company_claim()` (0018) | Solution découplée proposée puis retenue plutôt que de réécrire une fonction existante déjà éprouvée : zéro diff, zéro risque de régression sur le parcours de revendication (Phase 7) |
| 2026-09-20 | Sujet et message restent des champs libres saisis par l'utilisateur, jamais pré-remplis ni générés à partir du score de matching | Une demande de mise en relation doit représenter une intention humaine explicite, pas un texte automatique déguisé en message personnel (§6 de la demande) |
| 2026-09-20 | Notifications de mise en relation envoyées uniquement aux propriétaires/administrateurs actifs de l'entreprise (pas aux simples membres), messages génériques sans nom d'entreprise | Cohérent avec le principe déjà en place pour les autres notifications (Phase 5) ; éviter une requête de résolution de nom supplémentaire à chaque notification — asymétrie documentée, pas silencieuse (voir docs/PARTNERSHIP_REQUESTS.md §5/§7) |
| 2026-09-20 | Vérification factuelle demandée avant toute Phase 9 : une entreprise sans offre ni besoin ne peut recevoir aucun score de matching — confirmé **impossible**, verrouillé par un nouveau test de régression, aucun code de scoring modifié | Demande explicite de vérifier avant d'agir (§10 de la demande) ; le mécanisme de valeur neutre (50 %) ne s'applique qu'à un sous-critère manquant DANS une comparaison besoin↔offre déjà identifiée, jamais à l'absence du besoin/offre lui-même |
| 2026-09-20 | Acceptation d'une demande ne révèle que le statut « acceptée », jamais une coordonnée personnelle (courriel/téléphone/adresse) | Demande explicite (§8) ; déverrouillage de coordonnées privées à l'acceptation volontairement reporté à une décision future |
