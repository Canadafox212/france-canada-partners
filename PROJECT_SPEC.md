...
**Statut** : Phase 9 — mise en relation commerciale entre entreprises construite et testée : `partnership_requests` (migration `0023_partnership_requests.sql`, **appliquée sur Supabase**), 4 fonctions SECURITY DEFINER (création/acceptation/refus/retrait), gestion d'une entreprise cible non revendiquée sans notification automatique et sans modifier le parcours de revendication existant (Phase 7), interface complète (CTA sur la fiche publique et sur les partenaires potentiels, « Mes mises en relation » REÇUES/ENVOYÉES, page de détail, « Mes notifications ») ; 28 tests d'intégration dédiés + régression matching confirmant qu'une entreprise sans offre ni besoin ne peut recevoir aucun score. Aucune migration antérieure (0001-0022) modifiée. Voir docs/PARTNERSHIP_REQUESTS.md. Phase 8 (import réel des 13 entreprises françaises pilotes, 3 publiées) reste inchangée — voir §12 et le point 8 de §16.
