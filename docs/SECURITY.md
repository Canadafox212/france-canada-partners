...
## Corrections de sécurité issues d'un audit indépendant (Phase 10C, 2026-09-20/21)

Quatre lots, détaillés dans `CHANGELOG.md`. **Note de statut** : `0026` a des tests d'intégration réexécutés avec succès après application (24/24) ; `0024` et `0025` sont confirmées appliquées (objets réels vérifiés le 2026-09-23) mais leurs suites d'intégration dédiées n'ont pas été réexécutées depuis contre l'état actuel de la base — voir la dette documentée dans `CHANGELOG.md`.

**LOT 10C-2 — Détection de doublon et limitation anti-rafale** : `find_similar_companies()` (`SECURITY DEFINER`, réservée `authenticated`) ne retourne une entreprise `draft`/non active que sur un signal **fort** (numéro officiel + pays, domaine exact, ou nom exact + pays + ville) — jamais par simple sous-chaîne de nom — pour ne jamais servir d'annuaire de fiches non publiques ; recherche par sous-chaîne via `position()`, jamais `ilike` avec un joker fourni par l'utilisateur (un `'%'` aurait permis de lister toutes les entreprises). `create_partnership_request()` (0023) redéfinie pour ajouter un plafond de 20 demandes/heure glissante par **entreprise demandeuse** (`requester_company_id`, jamais par utilisateur), sérialisé par `pg_advisory_xact_lock()` pour empêcher qu'un simple `COUNT` puis `INSERT` non atomique laisse passer plus de 20 lignes sous appels concurrents.

**LOT 10C-3 — Protection des coordonnées professionnelles** : `companies` reste lisible publiquement au niveau ligne pour toute entreprise `active` (la RLS ne filtre jamais par colonne) — `professional_email`/`phone` y résidaient donc en clair et étaient **réellement affichées sur la fiche publique** jusqu'à ce lot. Déplacées vers `company_contacts` (RLS lecture membres/admin, écriture owner/admin, aucune politique de suppression) ; migration séquencée pour ne jamais laisser de fenêtre de lecture en clair (copie puis vérification ligne par ligne AVANT toute nullification, échec de la vérification annule toute la migration) ; anciennes colonnes verrouillées par une contrainte `CHECK` empêchant toute future écriture, plutôt qu'un trigger correcteur silencieux.

**LOT 10C-4 — Publication/activation sécurisée** : faille corrigée (audit indépendant Cowork) — `companies_update_owner_admin` (0002) n'a pas de `WITH CHECK`, un owner/admin pouvait donc écrire `companies.status = 'active'` par un simple `UPDATE`, sans validation. Nouvelle table `company_publication_requests` (même principe que `company_claims`/`partnership_requests` : **aucune** politique RLS d'écriture, tout passe par `request_company_publication()`/`review_company_publication_request()`, `SECURITY DEFINER`). Critères de préparation centralisés dans une fonction interne jamais exposée directement (`is_company_ready_for_publication()`, `REVOKE ALL FROM PUBLIC`, aucun `GRANT`) ; seul `admin_is_company_ready_for_publication()` (réservée `is_platform_admin()`) en expose le résultat à un rôle HTTP. `protect_companies_platform_fields()` (0009, fonction redéfinie) étend la protection déjà en place pour `subscription_level`/`verification_status` à `status` — un administrateur de plateforme garde un accès de secours par `UPDATE` direct, mais jamais silencieux (`log_audit_event()` systématique). La politique `companies_insert_authenticated` (0011) est resserrée pour contraindre aussi les valeurs `status`/`verification_status`/`subscription_level` d'un `INSERT`, pas seulement l'authentification de son auteur.

## Tests de sécurité réels (Phases 3 à 9, puis Phase 10C)
...
- `duplicateSearch.test.ts` (Phase 10C, 0024) : signal fort requis pour retourner une entreprise `draft`, recherche floue sur une entreprise `active`, longueur minimale, aucun paramètre significatif → aucune ligne, aucune coordonnée privée exposée.
- `partnershipRequestsRateLimit.test.ts` (Phase 10C, 0024) : seuil de 20 demandes/heure glissante par entreprise demandeuse, comportement sous appels concurrents.
- `companyContacts.test.ts` (Phase 10C, 0025) : RLS de `company_contacts` (membres/admin en lecture, owner/admin en écriture, aucun accès tiers/anonyme), non-exposition sur la fiche publique.
- `companyPublication.test.ts` (Phase 10C, 0026) : cycle de vie `draft`→`pending`→`approved`/`rejected`, revalidation à l'approbation sans rejet automatique, verrous `FOR UPDATE` contre deux décisions concurrentes, index unique partiel contre deux demandes `pending` simultanées, protection de `companies.status` contre une écriture directe par un owner/admin.

**Statut de ces 4 derniers fichiers** : écrits en même temps que leur migration (même pratique que toutes les phases précédentes), mais seul `companyPublication.test.ts` a un résultat vert réexécuté avec succès après application de sa migration (0026). Les trois autres n'ont pas été relancés depuis contre l'état actuel de la base — dette documentée dans `CHANGELOG.md`, à résoudre avant toute nouvelle migration touchant `find_similar_companies()`, `company_contacts` ou la limitation anti-rafale.
